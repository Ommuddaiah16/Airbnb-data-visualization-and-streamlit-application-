...
def datafr():
    df = pd.read_csv("C:\\Users\\igomp\\OneDrive\\Desktop\\Projects_Data_scince\\Airbnb_project\\airbnb.csv")
    return df
...
if select == "Data Exploration":
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["***PRICE ANALYSIS***", "***AVAILABILITY ANALYSIS***", "***LOCATION BASED***", "***GEOSPATIAL VISUALIZATION***", "***TOP CHARTS***"])
    with tab1:
        ...
        with col2:
            ...
            df_pie = pd.DataFrame(
                df4.groupby("host_response_time")[["price", "bedrooms"]].sum())
        ...
        with col2:
            ...
            df_do_bar_2 = pd.DataFrame(
                df5.groupby("bed_type")[["bedrooms", "beds", "accommodates", "price"]].sum())
    ...
    with tab2:
        ...
        def datafr():
            df_a = pd.read_csv("C:\\Users\\igomp\\OneDrive\\Desktop\\Projects_Data_scince\\Airbnb_project\\airbnb.csv")
            return df_a
        ...
        df_mul_bar_a = pd.DataFrame(
            df3_a.groupby("host_response_time")[["availability_30", "availability_60", "availability_90",
                                                  "availability_365", "price"]].sum())
        df_mul_bar_a.reset_index(inplace=True)

        fig_df_mul_bar_a = px.bar(df_mul_bar_a, x='host_response_time', y=['availability_30', 'availability_60',
    ...
    with tab3:
        ...
        sel_val = st.selectbox("Select the Value", [str(df2_l['price'].min()) + ' ' + str('to') + ' ' + str(
            differ_max_min * 0.30 + df2_l['price'].min()) + ' ' + str("(30% of the Value)"),
        ...
        df_sel = select_the_df(sel_val)

        fig_map_1 = px.scatter_mapbox(df_sel, lat="latitude", lon="longitude", hover_name="property_type",
                                       hover_data=["price", "room_type", "bed_type"],
                                       color_discrete_sequence=["fuchsia"], zoom=10, height=500, width=600)
    ...
    with tab4:
        ...
        def datafr():
            df_g = pd.read_csv("C:\\Users\\igomp\\OneDrive\\Desktop\\Projects_Data_scince\\Airbnb_project\\airbnb.csv")
            return df_g

        df_g = datafr()

        country_g = st.selectbox("Select the Country_g", df_g["country"].unique())

        df1_g = df_g[df_g["country"] == country_g]
        df1_g.reset_index(drop=True, inplace=True)

        proper_ty_g = st.selectbox("Select the Property_type_g", df1_g["property_type"].unique())

        df2_g = df1_g[df1_g["property_type"] == proper_ty_g]
        df2_g.reset_index(drop=True, inplace=True)

        def select_the_df(sel_val_g):
            if sel_val_g == str(df2_g['price'].min()) + ' ' + str('to') + ' ' + str(
                    differ_max_min_g * 0.30 + df2_g['price'].min()) + ' ' + str("(30% of the Value)"):

                df_val_30_g = df2_g[df2_g["price"] <= differ_max_min_g * 0.30 + df2_g['price'].min()]
                df_val_30_g.reset_index(drop=True, inplace=True)
                return df_val_30_g

            elif sel_val_g == str(differ_max_min_g * 0.30 + df2_g['price'].min()) + ' ' + str('to') + ' ' + str(
                    differ_max_min_g * 0.60 + df2_g['price'].min()) + ' ' + str("(30% to 60% of the Value)"):

                df_val_60_g = df2_g[df2_g["price"] >= differ_max_min_g * 0.30 + df2_g['price'].min()]
                df_val_60_1_g = df_val_60_g[df_val_60_g["price"] <= differ_max_min_g * 0.60 + df2_g['price'].min()]
                df_val_60_1_g.reset_index(drop=True, inplace=True)
                return df_val_60_1_g

            elif sel_val_g == str(differ_max_min_g * 0.60 + df2_g['price'].min()) + ' ' + str('to') + ' ' + str(
                    df2_g['price'].max()) + ' ' + str("(60% to 100% of the Value)"):

                df_val_100_g = df2_g[df2_g["price"] >= differ_max_min_g * 0.60 + df2_g['price'].min()]
                df_val_100_g.reset_index(drop=True, inplace=True)
                return df_val_100_g

        differ_max_min_g = df2_g['price'].max() - df2_g['price'].min()

        sel_val_g = st.selectbox("Select the Value_g", [str(df2_g['price'].min()) + ' ' + str('to') + ' ' + str(
            differ_max_min_g * 0.30 + df2_g['price'].min()) + ' ' + str("(30% of the Value)"),
                                                        str(differ_max_min_g * 0.30 + df2_g['price'].min()) + ' ' + str(
                                                            'to') + ' ' + str(
                                                            differ_max_min_g * 0.60 + df2_g['price'].min()) + ' ' + str(
                                                            "(30% to 60% of the Value)"),
                                                        str(differ_max_min_g * 0.60 + df2_g['price'].min()) + ' ' + str(
                                                            'to') + ' ' + str(
                                                            df2_g['price'].max()) + ' ' + str(
                                                            "(60% to 100% of the Value)")])

        df_sel_g = select_the_df(sel_val_g)

        fig_map_2 = px.scatter_mapbox(df_sel_g, lat="latitude", lon="longitude", hover_name="property_type",
                                      hover_data=["price", "room_type", "bed_type"],
                                      color_discrete_sequence=["blue"], zoom=10, height=500, width=600)
